"""
Phase 3 (VAE) - contamination sweep, ADIM 3: train the final architecture
(latent_dim=10, beta=0.25 fixed - see phase3_vae/phase3_vae_autoencoder.ipynb
section 9) on each of the 6 contaminated train sets, 5 weight-init seeds each
(30 models total).

The VAE class below is copied verbatim from the notebook's section 2 (`VAE`,
not the annealing-capable `VAE2` used only for the beta sweep in section 9) -
beta is fixed for the whole run here, matching the final chosen variant.

Training is unsupervised: is_attack is never fed to the model, only used
afterwards to build threshold.json and (in evaluate_contamination_sweep.py)
metrics. The early-stopping/threshold monitor set (val_benign.csv) is the
held-out benign split from ADIM 1 - disjoint from every train set and from
the fixed test set.
"""
import json
import time
...
def main() -> None:
    ...
    print(f"Threshold/early-stop validation set: {len(X_val_benign)} benign flows (held out, "
          f"disjoint from all train sets and the fixed test set)")
    ...
    print(f"\nDone: {len(run_log)} models trained "
          f"({len(CONTAM_LEVELS_PCT)} levels x {len(SEEDS)} seeds). "
          f"Log: {MODEL_DIR / 'training_run_log.json'}")
