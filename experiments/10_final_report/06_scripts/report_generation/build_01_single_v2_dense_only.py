"""
STAGE 1 scope: report figures for Dense v2 ONLY (single_attack_type). VAE v2
and pairwise/segmented v2 are later stages of the phased concurrency_src_1s
rollout (see CHANGELOG.md) -- not run here.

Mirrors build_01_single.py's make_roc_pr_figure()/run_model() pattern
exactly (same report_style, same figure layout) but points at the v2
(19-feature) Dense backend instead of v1. Old v1 report
(10_final_report/01_single_attack_type/) is untouched; this writes only to
10_final_report/01_single_attack_type/dense_v2/.
"""
import os
import shutil
import sys
...
from dense_backend_v2 import DEFAULT_DENSE_V2_BACKEND, MODEL_LABEL, load_feature_cols_v2  # noqa: E402
...
print("=== Dense v2 (5 seeds, +concurrency_src_1s) ===")
run_model(
    MODEL_LABEL, DEFAULT_DENSE_V2_BACKEND, os.path.join(OUT_ROOT, "dense"),
    os.path.join(DENSE_V2_DIR, "results_single_attack_type_dense_v2.csv"),
    os.path.join(DENSE_V2_DIR, "results_single_attack_type_dense_v2.md"),
)
print("done (Stage 1: Dense v2 only -- VAE v2 comes in a later stage)")
