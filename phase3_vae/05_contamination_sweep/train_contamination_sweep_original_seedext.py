"""
Phase 3 (VAE) - contamination sweep SEED-EXTENSION for the ORIGINAL 6
controlled-injection levels (0/1/2/4/8/12%): the resampled levels
(15/20/22%-clean) were bumped from 5 to 20 seeds and this revealed a
bimodal seed distribution (~10-20% of seeds land in a low PR-AUC cluster)
that a 5-seed sample can't reliably detect - std at 5 seeds looked
artificially narrow. This script checks whether the same phenomenon is
present in the original 6 levels, which have only ever been run at 5 seeds.

Architecture/hyperparameters copied verbatim from train_contamination_sweep.py
(same VAE class, same latent_dim=10, beta=0.25, same train/val split) - only
SEEDS differs (5-19 instead of 0-4). Existing seed_0..seed_4 models under
04_models/contam_{0,1,2,4,8,12}pct/ are NOT retrained - build_and_train() is
seeded per-call via tf.keras.utils.set_random_seed(seed), so re-running
seed 0-4 would reproduce the exact same weights anyway, but we skip via an
existence-check regardless (same pattern as train_contamination_sweep_extended.py).
"""
import json
import time
...
def main() -> None:
    ...
    for level_pct in CONTAM_LEVELS_PCT:
        ...
        print(f"\n=== contamination level {level_pct}% (seed-extension 5-19) "
    ...
    log_path = MODEL_DIR / "training_run_log_original_seedext.json"
